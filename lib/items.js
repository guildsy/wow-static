...
function fetchXML(id, cb) {
	var url = `http://${prefix}.wowhead.com/item=` + id + '?xml';
	...
	request(url, function(err, res, body) {
		...
		parseXML(body, function(err, obj) {
			...
			var data = {
				...
				isClassSpecific: item.htmlTooltip[0]?.includes('wowhead.com/class=')
			};
			...
			if (isConduit) {
				const match = item.htmlTooltip[0]?.match(/Requires ([a-zA-Z]+)/);
